      ...
      <BottomSheet ref={ref} snapPoints={snapPoints} onDismiss={onDismiss}>
        <View className="flex-1 px-4 py-2">
          ...
          {actions.map((action, index) => {
            ...
            const textColor = action.destructive ? tokens.colors.semantic.error : activeColors.text.primary;
            
            return (
              <TouchableOpacity
                key={action.id}
                onPress={() => {
                  // Wait for bottom sheet to close visually before executing action if needed, or just execute
                  action.onPress();
                }}
                className="flex-row items-center py-4"
                style={[
                  index !== actions.length - 1 && { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: activeColors.border }
                ]}
              >
                {Icon && (
                  <View className="mr-3">
                ...
                <Text 
                  ...
                  style={{ color: textColor, fontSize: tokens.typography.size.base }}
                >
                  {action.label}
                </Text>
              </TouchableOpacity>
