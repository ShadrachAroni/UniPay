...
import { ArrowUp, ArrowDown, Building2, Smartphone, Wallet, Star, CheckCircle2 } from 'lucide-react-native';
...
export default function MoneyDirectionScreen() {
  ...
  const setAsPrimary = async (targetIndex: number) => {
    ...
    setRules(newRules);
    showToast(`${selected.destination_type.replace('_', ' ').toUpperCase()} set as Primary Preferred Choice`, 'success');
  };
    ...
    <View className="flex-1" style={{ backgroundColor: activeColors.background }}>
      ...
      >
        <Text style={{ color: activeColors.text.secondary, fontSize: tokens.typography.size.base, marginBottom: tokens.spacing.xl }}>
          Choose your <Text className="font-bold" style={{ color: activeColors.text.primary }}>Preferred Primary Choice</Text> where incoming funds arrive first. Priority rules execute top-to-bottom.
        </Text>
        ...
        {rules.map((rule, index) => {
          ...
          return (
            <View 
              ...
              style={{ backgroundColor: isPrimary ? (isDark ? 'rgba(30, 58, 138, 0.3)' : '#eff6ff') : activeColors.surface }}
            >
              <View className="flex-row items-center justify-between mb-2">
                <View className="flex-row items-center">
                  ...
                  <View>
                    <Text className={`font-bold ${isPrimary ? 'text-base text-blue-900 dark:text-blue-200' : 'text-sm'}`} style={{ color: isPrimary ? undefined : activeColors.text.primary }}>
                    ...
                    <Text style={{ color: activeColors.text.secondary, fontSize: tokens.typography.size.xs }}>
                      {isPrimary ? 'Default Primary Destination' : `Fallback Priority #${index + 1}`}
                    </Text>
                  </View>
                </View>
                ...
                {isPrimary ? (
                  <View className="px-3 py-1 rounded-full bg-blue-600 flex-row items-center">
                    <Star size={12} color="#ffffff" className="mr-1" />
                    <Text className="text-[10px] font-bold text-white uppercase ml-1">PREFERRED</Text>
                  </View>
                ) : (
                  <TouchableOpacity
                  ...
                  >
                    <Text className="text-[11px] font-semibold text-blue-600 dark:text-blue-400">Set as Primary</Text>
                  </TouchableOpacity>
                )}
              </View>

              {/* Priority Reordering Controls & Allocation Details */}
              <View className="flex-row items-center justify-between pt-3 mt-1 border-t border-gray-200/60 dark:border-slate-800/60">
                <View className="flex-row items-center">
                  <Text style={{ color: activeColors.text.secondary, fontSize: tokens.typography.size.xs, marginRight: 8 }}>
                    Allocation:
                  </Text>
                  <Text className="font-bold text-xs" style={{ color: activeColors.text.primary }}>
                    {rule.allocation_type === 'percentage' ? `${rule.allocation_value}% of incoming` : `Fixed KES ${rule.allocation_value.toLocaleString()}`}
                  </Text>
                </View>
