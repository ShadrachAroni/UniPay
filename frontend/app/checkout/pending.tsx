...
export default function GuestCheckoutPendingScreen() {
  const params = useLocalSearchParams<{ intentId: string; payerPhone: string; amount: string; recipientName: string }>();
  const { tokens, isDark } = useTheme();
  const activeColors = isDark ? tokens.colors.dark : tokens.colors.light;
  const router = useRouter();

  useEffect(() => {
    // Poll/Simulate STK Push confirmation after 2.5 seconds
  ...
  return (
    <View className="flex-1 items-center justify-center p-6" style={{ backgroundColor: activeColors.background }}>
      ...
      <Text className="text-center text-sm px-6 mb-8 leading-6" style={{ color: activeColors.text.secondary }}>
        An M-PESA payment prompt has been sent to <Text className="font-bold" style={{ color: activeColors.text.primary }}>{params.payerPhone || 'your phone'}</Text>. Enter your PIN to complete paying <Text className="font-bold" style={{ color: activeColors.text.primary }}>{params.recipientName}</Text>.
      </Text>
