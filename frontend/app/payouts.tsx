...
import { View, Text, FlatList, TouchableOpacity, RefreshControl, TextInput, KeyboardAvoidingView, Platform, ScrollView } from 'react-native';
    ...
    return (
      ...
      >
        <View className="flex-row items-center flex-1">
          <View className="w-10 h-10 rounded-full items-center justify-center mr-3" style={{ backgroundColor: isDark ? tokens.colors.dark.border : '#f1f5f9' }}>
        ...
        <View className="items-end">
          ...
          <Chip label={item.status} style={{ backgroundColor: statusColor }} />
        </View>
      </View>
    );
  };
    ...
    <View className="flex-1" style={{ backgroundColor: activeColors.background }}>
      ...
      <BottomSheet ref={bottomSheetRef} snapPoints={['50%']}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 px-4 py-2">
          <Text className="font-bold mb-4" style={{ color: activeColors.text.primary, fontSize: tokens.typography.size.lg }}>
            Request Payout
          </Text>
