    ...
    async function load() {
      try {
        const [poolData, contribData] = await Promise.all([
          getPoolDetails(String(id)),
          getPoolContributions(String(id)),
        ]);
        setPool(poolData);
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: activeColors.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: tokens.spacing.lg }}>
        ...
        >
          <View className="flex-row justify-between items-start mb-4">
            ...
            <Chip
              ...
              style={{
                backgroundColor:
                  pool.status === 'open'
                    ? tokens.colors.status.pool.open
                    : tokens.colors.status.pool.closed,
              }}
          ...
          <View className="mb-4">
            <Text
              ...
              style={{ color: tokens.colors.light.brand, fontSize: tokens.typography.size['2xl'] }}
            >
        ...
        <Text
          className="font-semibold mb-4 ml-1"
